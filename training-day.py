...
Two mutually exclusive modes are supported:
  ...
  Single-label mode - tag every record from one input directory with the
  same label:
      --input DIR --label {0,1} --output DIR

  Mixed mode - build one dataset out of two input directories, labelling
  records from --positive as 1 and records from --negative as 0:
      --positive DIR --negative DIR --output DIR
...
def generate_processed_sentences(input_dir, description=None):
    """Run the read/tokenize/co-occurrence/sentence-generation pipeline for
    one input directory. Returns (processed_sentences, source_files)."""
    ...
    print(f"Reading input files from: {input_dir}{tag}")
    sentences, source_files = read_input_files(input_dir)
    print(f"Total lines loaded: {len(sentences)}")

    random.shuffle(sentences)

    print(f"Tokenizing{tag}...")
    all_text = ' '.join(sentences)
    tokens = extract_tokens(all_text)

    if not tokens:
        print(f"No tokens found in input{tag}. No output written.")
        sys.exit(1)

    print(f"  Building co-occurrence matrix ({len(tokens)} tokens){tag}...")
    matrix = calculate_cooccurrence(tokens)

    print(f"  Generating related sentences{tag}...")
    raw = create_related_sentences(tokens, matrix)
    raw = [s[0].upper() + s[1:] if s else s for s in raw]

    processed = post_process(raw)
    print(f"Records after post-processing{tag}: {len(processed)}")

    return processed, source_files
...
def main():
    ...
    parser.add_argument('--input',
                        help='Input directory of text files (single-label mode, requires --label)')
    parser.add_argument('--label', choices=['0', '1'],
                        help='Label to tag every record from --input with: '
                             '0 or 1 (single-label mode, requires --input)')

    parser.add_argument('--positive',
                        help='Input directory of text files to label 1 (mixed mode, requires --negative)')
    parser.add_argument('--negative',
                        help='Input directory of text files to label 0 (mixed mode, requires --positive)')

    args = parser.parse_args()

    single_mode_args = (args.input is not None) or (args.label is not None)
    mixed_mode_args = (args.positive is not None) or (args.negative is not None)

    if single_mode_args and mixed_mode_args:
        parser.error('use either --input/--label OR --positive/--negative, not both')
    elif single_mode_args:
        if args.input is None or args.label is None:
            parser.error('single-label mode requires both --input and --label')
    elif mixed_mode_args:
        if args.positive is None or args.negative is None:
            parser.error('mixed mode requires both --positive and --negative')
    else:
        parser.error('specify either --input/--label OR --positive/--negative')
    ...
    if single_mode_args:
        ...
        processed, source_files = generate_processed_sentences(input_dir)
        labels = [label_value] * len(processed)
        label_tag = str(label_value)
    else:
        positive_dir = Path(args.positive).resolve()
        negative_dir = Path(args.negative).resolve()

        pos_processed, pos_files = generate_processed_sentences(positive_dir, description='positive')
        neg_processed, neg_files = generate_processed_sentences(negative_dir, description='negative')

        processed = pos_processed + neg_processed
        labels = [1] * len(pos_processed) + [0] * len(neg_processed)
        source_files = pos_files + neg_files
        label_tag = 'mixed'

        combined = list(zip(processed, labels))
        random.shuffle(combined)
        processed, labels = map(list, zip(*combined)) if combined else ([], [])

    print(f"Total records available: {len(processed)}")
    ...
    for i in range(0, len(processed), bundle_size):
        ...
        data = build_pytorch_json(chunk_s, chunk_l, source_files, label_tag)
